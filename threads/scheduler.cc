...
static int L1Comparison(Thread* , Thread* );
static int L2Comparison(Thread* , Thread* );
//<TODO>
...
Scheduler::FindNextToRun ()
{
    ...
    if (!L1ReadyQueue->IsEmpty()) {
        DEBUG(dbgMLFQ, "[RemoveFromQueue] Tick [" << stats->totalTicks << "]: Thread [" 
            <<  L1ReadyQueue->Front()->getID() << "] is removed from queue L1");
        return L1ReadyQueue->RemoveFront();
    } else if (!L2ReadyQueue->IsEmpty()) {
        DEBUG(dbgMLFQ, "[RemoveFromQueue] Tick [" << stats->totalTicks << "]: Thread [" 
            <<  L2ReadyQueue->Front()->getID() << "] is removed from queue L2");
        return L2ReadyQueue->RemoveFront();
    } else if (!L3ReadyQueue->IsEmpty()) {
        DEBUG(dbgMLFQ, "[RemoveFromQueue] Tick [" << stats->totalTicks << "]: Thread [" 
            <<  L3ReadyQueue->Front()->getID() << "] is removed from queue L3");
...
//	cout << "Current Thread" <<oldThread->getName() << "    Next Thread"<<nextThread->getName()<<endl;
    ...
    DEBUG(dbgMLFQ, "Switching from: " << oldThread->getID() << " to: " << nextThread->getID());
...
static int L1Comparison(Thread* ThreadA, Thread* ThreadB)
{
    if (ThreadA->getRemainingBurstTime() == ThreadB->getRemainingBurstTime())
        return ThreadB->getPriority() - ThreadA->getPriority();
    return ThreadA->getRemainingBurstTime() - ThreadB->getRemainingBurstTime();
}

static int L2Comparison(Thread* ThreadA, Thread* ThreadB)
{
    return ThreadA->getID() - ThreadB->getID();
}
...
Scheduler::UpdatePriority()
{
    ...
    for (iter = new ListIterator<Thread* > (L2ReadyQueue); !iter->IsDone(); iter->Next()) {
        ...
        if (thread->getPriority() >= 100) {
            DEBUG(dbgMLFQ, "[RemoveFromQueue] Tick [" << stats->totalTicks << "]: Thread [" 
                << thread->getID() << "] is removed from queue L2");
    ...
    for (iter = new ListIterator<Thread* > (L3ReadyQueue); !iter->IsDone(); iter->Next()) {
        ...
        if (thread->getPriority() >= 50) {
            DEBUG(dbgMLFQ, "[RemoveFromQueue] Tick [" << stats->totalTicks << "]: Thread [" 
                << thread->getID() << "] is removed from queue L3");
